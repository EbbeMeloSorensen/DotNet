...
namespace DMI.SMS.Persistence.Npgsql.Repositories
{
    public class ContactPersonRepository : IContactPersonRepository
    {
        ...
        {
            return await Task.Run(() => GetContactPersons(null));
        }

        public async Task<IEnumerable<ContactPerson>> Find(
            Expression<Func<ContactPerson, bool>> predicate)
        {
            return await Task.Run(() => GetContactPersons($" WHERE {predicate.ToMSSqlString()}"));
        }
